...
class EskulController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data = (new BaseApi)->index('/api/eskul', ['search_eskul' => $search]);
        $students = $data->json();
        // dd($students);
        return view('index')->with(['eskul' => $students['data']]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $proses = (new BaseApi)->store('/api/eskul/tambah-data', $data);
        if ($proses->failed()) {
    ...
    public function edit($id)
    {
        ...
        $data = (new BaseApi)->edit('/api/eskul/'.$id);
        if ($data->failed()) {
            ...
            return view('edit')->with(['eskul' => $student]);
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $proses = (new BaseApi)->update('/api/eskul/update/'.$id, $payload);
        if ($proses->failed()) {
    ...
    {
        $proses = (new BaseApi)->delete('/api/eskul/'.$id. '/delete');
        if ($proses->failed()) {
            $errors = $proses->json('data');
            return redirect()->back()->with(['errors' => $errors]);
        }else {
    ...
    {
        $proses = (new BaseApi)->trash('/api/eskul/show/trash');
        // dd($proses);
        if ($proses->failed()) {
            $errors = $proses->json('data');
            return redirect()->back()->with(['errors' => $errors]);
        } else {
            $eskulTrash = $proses->json('data');
            return view('trash')->with(['eskulTrash' => $eskulTrash]);
        }
    }

    public function permanent($id)
    {
        $proses = (new BaseApi)->permanent('/api/eskul/show/trash/permanent/'.$id);
        if ($proses->failed()) {
            $errors = $proses->json('data');
            return redirect()->back()->with(['errors' => $errors]);
        } else {
    ...
    {
        $proses = (new BaseApi)->restore('/api/eskul/trash/restore'.$id);
        if ($proses->failed()) {
            $errors = $proses->json('data');
            return redirect()->back()->with(['errors'  => $errors]);
        } else {
